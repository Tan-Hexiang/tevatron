...
with open(args.input, 'r') as f:
    data = f.readlines()
print("input len: {}".format(len(data)))
    ...
    for line in example:
        ...
        if result=={}:
            ...
            if args.save_ctxs_text:
                result['ctxs'][0]['text'] = corpus[int(docid)-1]['text']
                result['ctxs'][0]['title'] = corpus[int(docid)-1]['title']
        else:
            assert qid == result['id']
...
for i in tqdm(range(int(len(data)/100))):
    example = data[i*args.depth :(i+1)*args.depth]
