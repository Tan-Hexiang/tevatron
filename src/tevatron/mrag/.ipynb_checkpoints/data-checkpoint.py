...
class MPreProcessor:
    def __init__(self, dpr_tokenizer, fid_tokenizer, data_args: MDataArguments, mode='train'):
        self.data_args = data_args
        self.dpr_tokenizer = dpr_tokenizer
        self.fid_tokenizer = fid_tokenizer
        # self.corpus = load_dataset(data_args.corpus)['train']
        self.corpus = load_from_disk("tevatron_nq_corpus")['train']
        self.mode =mode
        if mode=='train':
            self.n_context = self.data_args.n_context
            logging.info("datasets use n_context {}".format(self.n_context))
        elif mode=='validate':
            self.n_context = self.data_args.val_n_context
    ...
    def __call__(self, example):
        ...
        dpr_question = self.dpr_tokenizer.encode(example['question'],
                                                 ...
                                                 truncation=True)
        # random sample an answer from answers list to train, raw_answers to validate
        raw_answers = example['answers']
        # transformers版本问题：https://github.com/AMontgomerie/question_generator/issues/6
        ...
        logging.debug("fid passages ids details: {}".format(fid_passage_ids))
        logging.debug("fid passages mask details: {}".format(fid_passage_mask))
        logging.debug("fid_passage_ids type {}".format(type(fid_passage_ids)))
        logging.debug("fid_passage_mask type {}".format(type(fid_passage_mask)))
        logging.debug("dpr_passages type {}".format(type(dpr_passages)))
        logging.debug("dpr_passages len {}".format(len(dpr_passages)))
...
class HFMTrainDataset:
    def __init__(self, dpr_tokenizer: PreTrainedTokenizer,
        ...
        self.mode = mode
        self.preprocessor = MPreProcessor(self.dpr_tokenizer, self.fid_tokenizer, data_args=self.data_args, mode=mode)
...
class MTrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode=='validate':
            result['raw_answers'] = group['raw_answers']
            result['scores'] = group['scores']
        return result


@dataclass
class MTrainCollator:
    def __init__(self, in_batch_negative, model_args, data_args, mode='train') -> None:
        self.in_batch_negative = in_batch_negative
        self.mode = mode
        self.model_args = model_args
        self.data_args = data_args
        self.dpr_tokenizer = AutoTokenizer.from_pretrained(self.model_args.model_name_or_path)
    ...
    def __call__(self, batch):
        ...
        qq = [f['dpr_question'] for f in batch]
        dd = [f['dpr_passages'] for f in batch]
        ...
        q_collated = self.dpr_tokenizer.pad(
            qq,
            padding='max_length',
            max_length=self.data_args.dpr_query_len,
            return_tensors="pt",
        )
        d_collated = self.dpr_tokenizer.pad(
            dd,
            padding='max_length',
            max_length=self.data_args.dpr_passage_len,
            return_tensors="pt",
        ...
        result = {
            'dpr_question': q_collated,  # bsz, len
            'dpr_passages': d_collated,  # bsz, n_context, len
        ...
        if self.mode=='validate':
            # list[list[]]的形式，并不是tensor
            result['raw_answers'] = [x['raw_answers'] for x in batch]
            result['scores'] = [x['scores'] for x in batch]
