...
class MaskRetrievalAugmentGeneration(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.clip_gradients(optimizers[1], gradient_clip_val=5.0, gradient_clip_algorithm="norm")
        # step
        optimizers[0].step()
        optimizers[1].step()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        indices = indices[:, :k]
        # log expect
        # MIN, MAX = torch.min(sorted_sim), torch.max(sorted_sim)
        # s = 20 * ( (sorted_sim - MIN)/(MAX - MIN + 0.000001) - 0.5 ) + self.bias
        # dist = RectifiedStreched(
        #     BinaryConcrete(torch.full_like(s, 0.2), s), l=-0.1, r=1.1,
        # )
        # expected_l0 = dist.expected_L0()
        # logging.debug("sim : {}".format(sim))
        # logging.debug("scores in data : {}".format(batch['scores']))
        # logging.debug("sorted sim : {}".format(sorted_sim))
        # logging.debug("indices : {}".format(indices))
        # logging.debug("s : {}".format(s))
        # logging.debug("expectation : {}".format(expected_l0))
        ...
        for line in range(fid_a.shape[0]):
            # logging.debug("fid_p_ids[{}] shape: {}".format(line,fid_p_ids[line].shape))
            # logging.debug("selected shape {}".format(torch.index_select(fid_p_ids[line], dim=0, index=indices[line])))
            # logging.debug("fid_p_ids[{}]: {}".format(line, fid_p_ids[line]))
            # logging.debug("indices[{}] : {}".format(line, indices[line]))
            # logging.debug("new_fid_p_ids[{}] {}".format(line,torch.index_select(fid_p_ids[line], dim=0, index=indices[line])))
